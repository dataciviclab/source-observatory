...
def _scan_sitemap(
    ...
    *,
    ...
    # Build prefix/format stats from sample (deduplicated by URL)
    seen_urls: set[str] = set()
    deduped_links: list[dict[str, str]] = []
    for link in all_data_links:
        key = (link["url"], link.get("format"))
    ...
    for link in all_data_links:
        ...
        page_url = link.get("_page_url")
        page_title = page_meta.get(page_url, {}).get("title") if page_url else None
        ...
        rows.append({
            ...
            "organization": None,
            "tags": None,
            "notes_excerpt": page_meta.get(page_url, {}).get("description") if page_url else None,
