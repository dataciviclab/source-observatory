...
```
toolkit_source(action="probe", url=<URL>)       → status, content-type, reachability
```
...
Usa `toolkit_source(action="links")` per vedere se la pagina è HTML con link a dati:

```
toolkit_source(action="links", url=<URL>) → lista link, formati, total
```
...
```
toolkit_source(action="probe", url=<base_url>/api/3/action/package_list?limit=1)
   → se 200 + JSON: CKAN confermato
```

**CKAN — verifica enumerateabilità** (solo se confermato CKAN):
```
toolkit_source(action="ckan", endpoint=<base_url>, package_id='<un_package_id_noto>')
   → se success: catalog-watch confermato
```

**SDMX** — prova l'endpoint REST:
```
toolkit_source(action="probe", url=<base_url>/rest/dataflow)
   → se 200 + XML/JSON: SDMX confermato
```

**SPARQL** — verifica endpoint su path standard:
```
toolkit_source(action="probe", url=<base_url>/sparql?query=SELECT+1)
...
**HTML generico** — se CKAN/SPARQL/SDMX falliti ma `toolkit_source(action="links")` trova link CSV/JSON/XLSX:
```
