...
| inail_opendata | portal | aem | radar-only | YELLOW | 403 | - |
...
- `inail_opendata`: HTTP 403 | content-type: text/html | url finale: https://dati.inail.it/ | Portale AEM con Open API documentate. Superficie operativa reale = catalogo AEM + Open API, non CKAN standard (logo nel footer fuorviante). Baseline inventariale non ancora difendibile; tenere in radar-only finche' non definito un metodo stabile.
